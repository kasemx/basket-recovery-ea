#ifndef BRE_INF_LEGACY_MT5_TRADE_REQUEST_EXECUTOR_MQH
#define BRE_INF_LEGACY_MT5_TRADE_REQUEST_EXECUTOR_MQH

// Sprint 5 per-operation broker gateway. Retained for infrastructure unit tests only.
// Not registered in Bootstrapper/ServiceContainer. Superseded by ITradeExecutor + CMt5TradeExecutor (Sprint 6B).
...
class CLegacyMt5TradeRequestExecutor : public ITradeRequestExecutor
  {
private:
...
public:
                     CLegacyMt5TradeRequestExecutor(ILogger *logger=NULL)
     : m_builder(m_policy)
     {
      ...
      m_audit=CExecutionAuditLogger(logger);
                     ...
                     CLegacyMt5TradeRequestExecutor(const CExecutionPolicy &policy,ILogger *logger=NULL)
     : m_policy(policy),
       m_builder(policy)
     {
      m_audit=CExecutionAuditLogger(logger);
